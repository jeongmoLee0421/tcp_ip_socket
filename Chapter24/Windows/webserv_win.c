...
int main(int argc, char* argv[]) {
	...
	while (1) {
		...
		// request에 응답하기 위한 스레드 생성 후 실행 흐름 이동
...
unsigned WINAPI RequestHandler(void* arg) {
	...
	// 먼저 만나는 " /" 구분자를 찾아서 자른 후에 method에 복사
	...
	// 다음 문자열을 자르기 위해서 strtok 함수의 첫번째 인자로 NULL을 전달하면 된다.
...
void SendData(SOCKET sock, char* ct, char* fileName) {
	...
	if ((sendFile = fopen(fileName, "r")) == NULL) { // 파일 열기 실패
		SendErrorMSG(sock);
		return;
	}
